...
<div class="artist-detail">
	<h1>{$Artist->getParameter('ArtistFullName')}</h1>
	...
	{if $Images ne ""}
		...
		<a href="{$Image.Original}"><img border="0" src="{$Image.$Thumb}" /></a>
	{/if}
	<p class="artist-short-description">
		{$Artist->getParameter("ArtistDescription")}
	</p>
	<p class="artist-description">
		{$Artist->getParameter("ArtistLongDescription")|nl2br}
	</p>
	{if $Artist->getParameter('ArtistWebsite') ne ''}
	<p class="artist-website">
		<a href="{$Artist->getParameter('ArtistWebsiteURL')}" target="_blank">{$Artist->getParameter('ArtistWebsite')}</a>
	</p>
	{/if}
	...
	<h3><a href="{$SchedulePage}id={$Artist->getParameter('ArtistID')}">{'Artist'|vocabulary} Schedule</a></h3>
...
<div class="artist-detail-lineup">
	<h1>All {$Year} {'Artist'|pluralize}</h1>
	{include file='festivalapp.lineup_listing.tpl'}
</div>
