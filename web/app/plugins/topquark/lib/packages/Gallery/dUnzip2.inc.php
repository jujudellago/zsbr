	...
	Function _loadFileListByEOF(&$fh, $stopOnFile=false){
		...
		for($x = 0; $x < 1024; $x++){
			...
			if($signature == $this->dirSignatureE){
				...
				// Then, load file list
				if ($this->endOfCentral['offset_start_cd'] < 0){
				    // Need to deal with a large large number
				    $floatpos = floatval(sprintf('%u',$this->endOfCentral['offset_start_cd']));
				    $this->endOfCentral['offset_start_cd'] = $floatpos;
				}
				if ($this->endOfCentral['offset_start_cd'] > PHP_INT_MAX){
			        $offset = intval($this->endOfCentral['offset_start_cd'] - PHP_INT_MAX);
			        fseek($fh,PHP_INT_MAX);
