...
var multiBox = new Class({
	...
	options:{
		...
		triggerEvent: 'click', // click or dblclick to open the multibox
	...
	start: function(){
		...
		this.controlEffects = new Fx.Morph(this.controlsContainer,{duration:100});
		this.reset();
	},
	
	setContentOrder: function(){
		//start breaking into content array to add event listeners to each link within each group
		this.content.each(function(el,i){
			//now we are left with each group as arrays
			el.each(function(group,i){
				//add event listener
				group.removeEvents(this.options.triggerEvent);
				group.addEvent(this.options.triggerEvent, function(e){
	...
	showContent: function(){
		...
		if(this.type == 'image'){
			...
			new Element('iframe', {
				...
				'src': this.contentObj.url+'&noCache='+new Date().getTime(),
				'frameborder': 0,
				'scrolling': 'auto'
			...
			var req = new Request.HTML({
				url: this.contentObj.url+'&noCache='+new Date().getTime(),
				method: 'get',
				evalScripts: true,
		...
		this.contentEffects = new Fx.Morph(this.contentContainer,{duration:100});
		...
		if (this.options.onShowContent){
			this.options.onShowContent(this);
		}
	},
	...
	showControls: function(){
		...
		this.controlsContainer.setStyle('width',this.container.getStyle('width'));
	...
	next: function(){
		if(this.index < this.currentGallery.length-1){
			...
			this.timer = this.load.pass(this.currentGallery[this.index],this).delay(500);
